...
function FormFacade(data)
{
    ...
    this.getPrice = function(item, crncy='$')
    {
        ...
        var prc = {min:0};
        if(item.help && item.help.indexOf('${')<0)
        ...
        if(prc.min==0)
        {
            if(item.type=='LIST' || item.type=='MULTIPLE_CHOICE' || item.type=='SCALE' || item.type=='CHECKBOX')
            {
                if(item.choices)
                {
                    var chs = item.choices.map(ch=>formFacade.computeField('${price("'+ch.value+'","'+crncy+'")}'));
                    chs.sort((a,b)=>a-b);
                    prc.min = chs[0];
                    prc.max = chs.pop();
                }
            ...
            else if(item.type=='GRID')
            {
                if(item.rows)
                {
                    var chs = item.rows.map(ch=>formFacade.computeField('${price("'+ch.value+'","'+crncy+'")}'));
                    chs.sort((a,b)=>a-b);
                    prc.min = chs[0];
                    prc.max = chs.pop();
                }
        ...
        prc.minformat = formFacade.computeField('${format('+prc.min+',"'+crncy+'")}');
        if(prc.max) prc.maxformat = formFacade.computeField('${format('+prc.max+',"'+crncy+'")}');
        item.price = prc;
        return prc;
    }
    ...
    params.categories = params.CATEGORIES = function()
    {
        ...
        secs.forEach(function(sec, s){
            var prds = sec.items.filter(itm=>{
                ...
                var visible = true;
                var fitm = fac.items?fac.items[itm.id]:null;
